#!/usr/bin/env python3
"""Qwen3.5 MoE launcher core for upstream llama.cpp."""
...
def binary_path(value, mode):
    ...
    if not path.is_file():
        fail(f"Backend file not found: {path}")
...
def clean_environment():
    # Reproducible presets: stale LLAMA_ARG_* values must not silently alter
...
def cpu_counts():
    ...
    # Linux: prefer physical cores. On Windows/macOS we deliberately fall
    # back to os.cpu_count() instead of adding platform-specific dependencies.
...
def main():
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "Pass additional llama.cpp options after --. "
            "Later options may override the generated command."
        ),
    )
    p.add_argument("--version", action="version", version=f"Summer {VERSION}")
    p.add_argument("--mode", choices=("cli", "server"), default="cli")
    p.add_argument("--bin", required=True, help="path to llama-cli/llama-server executable")
    p.add_argument(
        "--model",
        required=True,
        help="Qwen3.5 MoE GGUF (first shard for split models)",
    )
    ...
    p.add_argument(
        "--fit-target",
        help="MiB to keep free per accelerator; default comes from profile",
    )
    p.add_argument(
        "--n-cpu-moe",
        help="keep routed-expert weights for the first N layers on CPU",
    )
    p.add_argument(
        "--cpu-moe",
        action="store_true",
        help="keep all MoE expert weights on CPU",
    )
    p.add_argument(
        "--ctk",
    ...
    ctx = as_int(args.ctx or min(defaults["ctx"], info["context_length"]), "ctx", 1)
    batch = as_int(args.batch or defaults["batch"], "batch", 1)
    ubatch = as_int(args.ubatch or min(defaults["ubatch"], batch), "ubatch", 1, batch)
    threads = as_int(args.threads or physical, "threads", 1)
    batch_threads = as_int(args.batch_threads or logical, "batch-threads", 1)
    fit_target = as_int(args.fit_target or defaults["fit_target"], "fit-target", 0)
    ctk = args.ctk or defaults["ctk"]
    ctv = args.ctv or defaults["ctv"]
    ...
    want_cpu_moe = args.cpu_moe or defaults["cpu_moe"]
    ...
    if args.n_cpu_moe is not None:
        ...
        command += ["--cpu-moe"]
        notes.append(
            "all MoE expert weights stay on CPU; "
            "VRAM use drops but token generation may slow down"
        )
    ...
    mtp_supported = (
        info["mtp_layers"] > 0
        and required_mtp <= flags
        and "draft-mtp" in help_text
    )

    if args.mtp == "on" and not mtp_supported:
        fail(
            "MTP was required, but the GGUF/backend does not expose "
            "compatible embedded draft-MTP support"
        )
    ...
    print(f"Summer {VERSION} — Qwen3.5 MoE / llama.cpp")
    ...
    if extra:
        print(
            "  note           : arguments after -- are appended verbatim "
            "and can override generated settings"
        )
